...
const withApiUrl = (payload) => ({ apiUrl: baseUrl, ...payload });
...
export const startServer = async (portNumber = defaultPort) => {
    await connectDatabase();
    return app.listen(portNumber, () => {
        console.log(`Backend listening on port ${portNumber}`);
        console.log(`API base URL: ${getApiBaseUrl(portNumber)}`);
    });
};
